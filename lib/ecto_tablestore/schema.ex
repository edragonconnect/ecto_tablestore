defmodule EctoTablestore.Schema do
  ...
  * Since Tablestore product does not support(also not recommend) an autogenerated primary key(aka the partition key)
  ...
  * Since Tablestore product's attribute column does not support `DateTime` type in server side, so we use `@timestamps_opts`
    to pre-configure `Ecto.Schema.timestamps/1` macro as an integer type for the generated `inserted_at` and `updated_at`
    fields.
  ...
  * If the partition key set as `autogenerate: true` is not allowed to take advantage of the
  ...
  @doc """
  This macro is reserved to make compatible when `:hashids` or `EctoTablestore.Hashids` type defined.
